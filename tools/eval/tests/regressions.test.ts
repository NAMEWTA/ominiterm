...
import { getPythonCommand } from "../src/runtime.ts";
...
async function withCommandOverride<T>(
  prefix: "PYTHON" | "CODEX" | "CLAUDE",
  scriptPath: string,
  fn: () => Promise<T>,
): Promise<T> {
  return withEnv(
    {
      [`OMINITERM_${prefix}_CMD`]: process.execPath,
      [`OMINITERM_${prefix}_ARGS`]: JSON.stringify([scriptPath]),
    },
    fn,
  );
}
...
describe("eval regressions", () => {
  it("parses SWE-bench result files named with model and run id", async (t) => {
    ...
    const pythonStub = await makeExecutable(
      binDir,
      "python3",
...
for (let i = 2; i < process.argv.length; i++) {
  if (process.argv[i] === "--run_id" && i + 1 < process.argv.length) {
    runId = process.argv[i + 1];
    i++;
  }
}

writeFileSync(
  "fake-model." + runId + ".json",
  JSON.stringify({ "task-parse": { resolved: true } }),
);
`,
    );

    const results = await withCommandOverride("PYTHON", pythonStub, () =>
      runSWEBenchEval({
        predictionsPath,
...
writeFileSync(process.env.CAPTURE_ARGS_FILE, process.argv.slice(2).join("\\n"));
...
writeFileSync("fake." + runId + ".json", JSON.stringify({}));
`,
    );

    async function captureDataset(benchmark: string, runId: string): Promise<string> {
      return withCommandOverride("PYTHON", pythonStub, () =>
        withEnv({ CAPTURE_ARGS_FILE: captureFile }, async () => {
          await runEvaluation(
            [],
            makeConfig({
              run_id: runId,
  ...
  it("parses cost from the last line of codex JSONL output", async () => {
    ...
    try {
      await mkdir(binDir, { recursive: true });
      const codexStub = await makeExecutable(
        binDir,
        "codex",
        `process.stdout.write(JSON.stringify({ event: "start" }) + "\\n");
process.stdout.write(JSON.stringify({ cost_usd: 1.75 }) + "\\n");
`,
      );
      const baseCommit = await initGitRepo(repoDir);
      const runner = new SingleCodexRunner();
      const result = await withCommandOverride("CODEX", codexStub, () =>
        runner.run(
...
writeFileSync(process.env.CODEX_LOG, process.argv.slice(2).join("\\n"));
process.stdout.write(JSON.stringify(["subtask"]) + "\\n");
`,
      );
      const claudeStub = await makeExecutable(
        binDir,
        "claude",
        `import { writeFileSync } from "node:fs";

writeFileSync(process.env.CLAUDE_LOG, process.argv.slice(2).join("\\n"));
process.stdout.write(JSON.stringify(["subtask"]) + "\\n");
`,
      );

      await withCommandOverride("CODEX", codexStub, () =>
        withCommandOverride("CLAUDE", claudeStub, () =>
          withEnv(
            {
              CODEX_LOG: codexLog,
  ...
  it("uses task-specific worktree paths and branch names for hydra sub-agents", async () => {
    ...
    try {
      await mkdir(binDir, { recursive: true });
      const codexStub = await makeExecutable(binDir, "codex", "");
      const baseA = await initGitRepo(repoA);
      const baseB = await initGitRepo(repoB);

      subA = await withCommandOverride("CODEX", codexStub, () =>
        runner.runSubAgent(
          "subtask",
      ...
      subB = await withCommandOverride("CODEX", codexStub, () =>
        runner.runSubAgent(
          "subtask",
