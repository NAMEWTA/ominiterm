...
import { getClaudeCommandSpec, getCodexCommandSpec } from "../runtime.ts";
...
/**
 * Direct Hydra runner — multi-agent orchestration without OminiTerm.
 *
 * Flow:
  ...
  private async decompose(
    ...
    try {
      if (
        ...
        const codex = getCodexCommandSpec();
      ...
      const claude = getClaudeCommandSpec();
      const { stdout } = await execWithStdin(
        claude.command,
        [
          ...claude.args,
          ...
          "--model",
          config.models.hydra_orchestrator_model,
        ],
        { cwd: workDir, timeout: 120, stdin: prompt },
      );
      return this.parseDecomposition(stdout);
      ...
      if (agentType === "codex") {
        ...
        await execWithStdin(
          ...
          [
            ...
            "--dangerously-skip-permissions",
            "--model",
            config.models.hydra_sub_claude_model,
          ],
          { cwd: worktreePath, timeout: timeoutS, stdin: prompt },
        );
      }
...
/** Original OminiTerm-based Hydra runner (for use within OminiTerm projects) */
export class OminiTermHydraRunner implements AgentRunner {
  async run(
    task: TaskDefinition,
    workDir: string,
    config: EvalConfig,
