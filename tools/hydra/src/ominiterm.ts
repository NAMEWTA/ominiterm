...
export function isOminiTermRunning(): boolean {
  try {
...
export function parseJsonOrDie(stdout: string): any {
  try {
    ...
    throw new Error(`Failed to parse OminiTerm response: ${stdout.slice(0, 200)}`);
  }
}
...
function runTermcanvasJson(args: string[], timeout: number): any {
  let stdout: string;
  try {
    stdout = execFileSync("ominiterm", args, {
      encoding: "utf-8",
      timeout,
    });
  } catch (err: any) {
    ...
    throw new Error(`ominiterm ${args.slice(0, 2).join(" ")} failed: ${detail}`);
  }
