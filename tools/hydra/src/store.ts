...
function normalizeAgentRecord(record: AgentRecord): AgentRecord {
  return {
    ...record,
    repo: path.resolve(record.repo),
    worktreePath: path.resolve(record.worktreePath),
  };
}
...
export function saveAgent(record: AgentRecord): void {
  fs.writeFileSync(agentPath(record.id), JSON.stringify(normalizeAgentRecord(record), null, 2));
}

export function loadAgent(id: string): AgentRecord | null {
  try {
    return normalizeAgentRecord(JSON.parse(fs.readFileSync(agentPath(id), "utf-8")) as AgentRecord);
  } catch {
    return null;
  }
}
    ...
    .map((f) => {
      try {
        return normalizeAgentRecord(
          JSON.parse(fs.readFileSync(path.join(dir, f), "utf-8")) as AgentRecord,
        );
      } catch {
        return null;
      }
