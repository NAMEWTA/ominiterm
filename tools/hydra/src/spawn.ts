...
export function spawn(args: string[]): void {
  ...
  // Validate OminiTerm is running
  if (!isOminiTermRunning()) {
    throw new Error("OminiTerm is not running");
  }

  // Validate repo is on canvas
  const project = findProjectByPath(repo);
  if (!project) {
    throw new Error(`Repo not found on OminiTerm canvas: ${repo}`);
  }
  ...
  if (parsed.worktree) {
    ...
    // Trigger OminiTerm to detect new worktree
  ...
  const parentTerminalId = process.env.OMINITERM_TERMINAL_ID;
