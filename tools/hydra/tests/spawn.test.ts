...
test("validateWorktreePath accepts repo root and nested worktrees", () => {
  ...
  const nestedWorktreePath = path.join(repoPath, ".worktrees", "existing");
  assert.equal(
    validateWorktreePath(repoPath, repoPath),
    repoPath,
  );
  assert.equal(
    validateWorktreePath(repoPath, nestedWorktreePath),
    nestedWorktreePath,
  );
});
...
test("validateWorktreePath rejects worktrees outside the repo", () => {
  const repoPath = path.resolve("tmp", "repo");
  const otherRepoPath = path.resolve("tmp", "other-repo");
  assert.throws(
    () => validateWorktreePath(repoPath, otherRepoPath),
