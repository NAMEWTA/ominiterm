...
const otherRepoPath = path.resolve("tmp", "other");
...
const record = {
  ...
  repo: repoPath,
  terminalId: "tc-001",
  worktreePath: path.join(repoPath, ".worktrees", "hydra-1234-abcd"),
...
test("listAgents filters by repo", () => {
  assert.equal(listAgents(repoPath).length, 1);
  assert.equal(listAgents(otherRepoPath).length, 0);
});
