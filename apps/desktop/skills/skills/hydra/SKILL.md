...
3. **You MUST poll all agents until every one reaches "completed" or "error".**
   ...
   **Adaptive polling interval** — scale with expected task duration:
   ...
   Use `sleep <seconds>` then check: `ominiterm terminal status <terminalId>`
   ...
   **Unblocking stalled agents**: If status is NOT "completed"/"error" AND
   ...
   `ominiterm terminal output <terminalId> --lines 20`
   ...
   turns, not stalled.
   `ominiterm terminal input <terminalId> $'\r'`
...
5. For code-change tasks only:
   - Review changes: `ominiterm diff <worktreePath> --summary`
