...
// The window.ominiterm.auth API is added by the preload agent in parallel.
...
export const useAuthStore = create<AuthStore>((set) => ({
  ...
  init: async () => {
    ...
    try {
      const [user, deviceId] = await Promise.all([
        // @ts-expect-error -- auth API added by preload agent
        window.ominiterm.auth.getUser(),
        // @ts-expect-error -- auth API added by preload agent
        window.ominiterm.auth.getDeviceId(),
      ]);
      set({ user, deviceId, loading: false });

      // Listen for auth state changes
      // @ts-expect-error -- auth API added by preload agent
      window.ominiterm.auth.onAuthStateChange((user: AuthUser | null) => {
        set({ user });
      });
    } catch {
      set({ loading: false });
  ...
  login: async () => {
    // @ts-expect-error -- auth API added by preload agent
    if (!window.ominiterm?.auth) return;
    set({ loginPending: true });
    // @ts-expect-error -- auth API added by preload agent
    const result = await window.ominiterm.auth.login();
    set({
      loginPending: false,
  ...
  logout: async () => {
    ...
    if (!window.ominiterm?.auth) return;
    // @ts-expect-error -- auth API added by preload agent
    await window.ominiterm.auth.logout();
    set({ user: null });
  },
