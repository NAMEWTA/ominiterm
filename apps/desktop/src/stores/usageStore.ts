...
export const useUsageStore = create<UsageStore>((set, get) => ({
  ...
  fetch: async (dateStr?: string) => {
    ...
    if (!window.ominiterm?.usage) {
      set({ loading: false });
      return;
    }

    try {
      const summary = await window.ominiterm.usage.query(target);
  ...
  fetchHeatmap: async () => {
    ...
    if (!window.ominiterm?.usage?.heatmap) {
    ...
    try {
      const data = await window.ominiterm.usage.heatmap();
  ...
  fetchCloud: async (dateStr?: string) => {
    ...
    if (!window.ominiterm?.usage?.queryCloud) {
      set({ cloudSummary: null });
      return;
    }
    try {
      // @ts-expect-error -- queryCloud will be added by the preload agent
      const data = await window.ominiterm.usage.queryCloud(target);
  ...
  fetchCloudHeatmap: async () => {
    ...
    if (!window.ominiterm?.usage?.heatmapCloud) {
      set({ cloudHeatmapData: null });
      return;
    }
    try {
      // @ts-expect-error -- heatmapCloud will be added by the preload agent
      const data = await window.ominiterm.usage.heatmapCloud();
