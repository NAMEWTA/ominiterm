...
function destroyTerminalPty(ptyId: number | null | undefined) {
  if (ptyId == null || !window.ominiterm?.terminal) {
    return;
  }
  void window.ominiterm.terminal.destroy(ptyId).catch((err) => {
...
export const useProjectStore = create<ProjectStore>((set) => ({
  ...
  setFocusedTerminal: (terminalId, options) => {
    ...
    if (terminalId && options?.focusComposer !== false) {
      ...
      if (composerEnabled) {
        window.dispatchEvent(new CustomEvent("ominiterm:focus-composer"));
      } else {
        window.dispatchEvent(
          new CustomEvent("ominiterm:focus-xterm", { detail: terminalId }),
        );
      }
    }
