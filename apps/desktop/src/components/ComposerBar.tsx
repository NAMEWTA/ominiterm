...
export function ComposerBar() {
  ...
  const composerRight = useUiShellStore((state) =>
    state.rightRailCollapsed ? COLLAPSED_TAB_WIDTH : RIGHT_RAIL_WIDTH,
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener("ominiterm:focus-composer", handleFocusComposer);
    return () => window.removeEventListener("ominiterm:focus-composer", handleFocusComposer);
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await window.ominiterm.composer.submit(request);
      if (!result.ok) {
            ...
            <textarea
              ...
              onKeyDown={(event) => {
                ...
                if (targetTerminal) {
                  ...
                  if (seq !== null) {
                    event.preventDefault();
                    window.ominiterm.terminal.input(targetTerminal.ptyId, seq);
                    return;
                  }
                }
