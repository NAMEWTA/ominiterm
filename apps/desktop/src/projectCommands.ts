...
export async function addProjectFromDialog(t: T) {
  ...
  try {
    dirPath = await window.ominiterm.project.selectDirectory();
  } catch (err) {
  ...
  let info: Awaited<ReturnType<typeof window.ominiterm.project.scan>>;
  try {
    info = await window.ominiterm.project.scan(dirPath);
  } catch (err) {
