...
const PORT_FILE = path.join(os.homedir(), ".ominiterm", "port");

function getPort(): number {
  try {
    return parseInt(fs.readFileSync(PORT_FILE, "utf-8").trim(), 10);
  } catch {
    console.error("OminiTerm is not running (no port file found).");
    process.exit(1);
  }
}
...
function request(method: string, urlPath: string, body?: any): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (err) => {
      console.error("Failed to connect to OminiTerm:", err.message);
      process.exit(1);
    });
  ...
  try {
    if (group === "project") {
      if (command === "add" && rest[0]) {
        ...
        console.log("Usage: ominiterm project <add|list|remove> [args]");
      }
      ...
      if (command === "create") {
        ...
        console.log(
          "Usage: ominiterm terminal <create|list|input|status|output|destroy|set-title> [args]",
        );
      }
