...
const AUTH_FILE = path.join(OMINITERM_DIR, "auth.json");
// Device ID is always in ~/.ominiterm/ (not ~/.ominiterm-dev/) so that
...
const DEVICE_ID_DIR = path.join(os.homedir(), ".ominiterm");
...
export async function handleAuthCallback(url: string): Promise<void> {
  ...
  try {
    ...
    // Supabase redirects with fragment: ominiterm://auth/callback#access_token=...&refresh_token=...
    // Or with error: ominiterm://auth/callback#error=xxx&error_description=yyy
