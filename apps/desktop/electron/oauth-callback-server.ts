...
<html>
<head><meta charset="utf-8"><title>OminiTerm Login</title>
<style>
...
(function() {
  ...
  if (query.get("code")) {
    msg.innerHTML = '<h2 class="success">Login successful!</h2>' +
      '<p>You can close this tab and return to OminiTerm.</p>';
    return;
  }
  ...
  server.on("error", (err: NodeJS.ErrnoException) => {
    if (err.code === "EADDRINUSE") {
      ...
      settle({
        type: "error",
        error: "port_in_use",
        description: `Port ${CALLBACK_PORT} is already in use. Close any other OminiTerm instances and try again.`,
      });
    } else {
