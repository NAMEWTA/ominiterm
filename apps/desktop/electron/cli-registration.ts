...
const CLI_NAMES = ["ominiterm", "hydra"] as const;
...
function broadcastWindowsEnvironmentChange(): void {
  const script = [
    "Add-Type -Namespace OminiTerm -Name NativeMethods -MemberDefinition @\"",
    "using System;",
    ...
    "[void][OminiTerm.NativeMethods]::SendMessageTimeout(",
    "  [IntPtr]0xffff,",
    "  0x001A,",
    "  [UIntPtr]::Zero,",
    "  \"Environment\",",
...
export function isCliRegistered(
  ...
  if (deps.platform === "linux") {
    const target = path.join(deps.homedir(), ".local", "bin", "ominiterm");
    try {
